...
def sync_company(sb, company_name: str):
    ...
    # 5.5 Fetch Executive KPIs
    log("  Fetching Executive KPIs from PnL and Balance Sheet...")
    try:
        pnl_xml = tally.export_profit_and_loss()
        bs_xml = tally.export_balance_sheet()
        if pnl_xml and bs_xml:
            ...
            # Cost of Sales is usually Opening + Purchases - Closing + Direct Expenses
            # In Tally, it might output "Cost of Sales :: " or we can calculate it manually
            # Let's extract "Cost of Sales" if available, else calculate manually
            cost_of_sales = 0
            ...
            sb.table("dashboard_metrics").upsert({
                "company_id": company_id,
                "dashboard_name": "Executive Summary",
                "metrics_data": metrics_data,
                "updated_at": datetime.datetime.now(datetime.timezone.utc).isoformat()
            }, on_conflict="company_id,dashboard_name").execute()
            log("  Executive KPIs synced successfully")
    except Exception as e:
        log(f"  Error fetching/syncing Executive KPIs: {e}", "WARN")
