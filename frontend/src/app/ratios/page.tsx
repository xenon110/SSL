...
export default function RatiosPage() {
  ...
  const fetchMetrics = async (startDate?: string, endDate?: string) => {
    ...
    setError(null);
    try {
      const url = new URL('/api/universal-metrics?type=ratios', window.location.origin);
  ...
  return (
    <div className="p-6 space-y-6">
      <GenericDashboardView title="Financial Ratios" data={data} onDateChange={(start, end) => fetchMetrics(start, end)} isLoading={isLoading} />
    </div>
  );
}
