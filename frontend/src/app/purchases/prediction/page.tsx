...
import { ArrowLeft, Activity, TrendingUp, AlertCircle, MapPin, DollarSign, RefreshCw, CheckCircle2, Package, Sparkles, ShieldAlert, BarChart3, HelpCircle, Layers, Users } from "lucide-react";
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, BarChart, Bar, Legend, Cell } from 'recharts';
import { Badge } from "@/components/ui/badge";

export default function PurchasesPrediction() {
  ...
  const fetchPredictionData = useCallback(async (force = false) => {
    try {
      ...
      const purchaseUrl = '/api/purchases' + (params.toString() ? '?' + params.toString() : '');
      const aiUrl = '/api/purchases/ai' + (params.toString() ? '?' + params.toString() : '');

      // Fetch historical purchases metrics
      const histRes = await fetch(purchaseUrl);
      if (!histRes.ok) throw new Error('Failed to retrieve historical procurement data.');
      ...
      if (!aiRes.ok) {
        ...
        throw new Error(errJson.error || 'Failed to generate AI procurement predictions.');
      }
      const aiData = await aiRes.json();
      setData(aiData);
    } catch (err: any) {
      console.error("Purchases prediction fetch error:", err);
      setError(err.message || 'An unexpected error occurred while generating procurement forecasts.');
    } finally {
      setIsLoading(false);
  ...
  const chartData = useMemo(() => {
    if (!historicalData?.purchaseTrend || !data?.predictedTrend) return [];
    
    const historical = (historicalData.purchaseTrend || []).map((h: any) => ({
      name: h.month,
      "Historical Spend": h.spend,
      "Forecasted Spend": null,
      type: 'Historical'
    }));

    const forecast = (data.predictedTrend || []).map((f: any) => ({
      name: f.month,
      "Historical Spend": null,
      "Forecasted Spend": f.predictedSpend,
      type: 'Forecast'
    }));

    const connectionPoint = historical.length > 0 ? [{
      name: historical[historical.length - 1].name,
      "Historical Spend": historical[historical.length - 1]["Historical Spend"],
      "Forecasted Spend": historical[historical.length - 1]["Historical Spend"],
      type: 'Historical'
    }] : [];
    ...
    return (
      <div className="flex h-screen flex-col items-center justify-center bg-slate-50 dark:bg-slate-950">
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <h3 className="text-xl font-bold text-slate-800 dark:text-slate-200 mt-2">Gemini AI Projecting Procurement Budgets...</h3>
          <p className="text-sm text-slate-400 max-w-sm">Aggregating vendor lists, supply concentrations, and debit returns rates from Tally ledger databases.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-4 md:p-8 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <Card className="border-0 shadow-lg bg-white dark:bg-slate-900 relative overflow-hidden">
          ...
          <CardHeader className="pb-2">
            <CardDescription className="text-xs uppercase font-bold tracking-wider text-slate-400">Projected Next-Q Spend</CardDescription>
            <CardTitle className="text-2xl font-extrabold text-indigo-600 tabular-nums">
              {formatCurrency(data?.summaryKpis?.nextQuarterSpend || 0)}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-xs text-slate-400">Procurement budget estimated for raw materials supply line.</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-0 shadow-lg bg-white dark:bg-slate-900 relative overflow-hidden">
          ...
          <CardHeader className="pb-2">
            <CardDescription className="text-xs uppercase font-bold tracking-wider text-slate-400">Predicted Spend Increase</CardDescription>
            <CardTitle className="text-2xl font-extrabold text-emerald-600 tabular-nums">
              +{data?.summaryKpis?.growthPercent || 0}%
            </CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-xs text-slate-400">Projected monthly procurement rate expansion.</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-0 shadow-lg bg-white dark:bg-slate-900 relative overflow-hidden">
          ...
          <CardHeader className="pb-2">
            <CardDescription className="text-xs uppercase font-bold tracking-wider text-slate-400">Vendor Concentration Risk</CardDescription>
            <CardTitle className="text-2xl font-extrabold text-rose-500 flex items-center gap-1.5">
              {data?.summaryKpis?.riskLevel || 'Medium'}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-xs text-slate-400">Identified supplier dependency levels relative to total spend.</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-0 shadow-lg bg-white dark:bg-slate-900 relative overflow-hidden">
          ...
          <CardHeader className="pb-2">
            <CardDescription className="text-xs uppercase font-bold tracking-wider text-slate-400">Predicted Spend Peak</CardDescription>
            <CardTitle className="text-2xl font-extrabold text-amber-600">
              {data?.summaryKpis?.peakMonth || 'N/A'}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-xs text-slate-400">Projected month of highest raw materials outflow.</p>
          </CardContent>
        </Card>
      </div>

      {/* Main Projections Area Chart */}
      <Card className="border-0 shadow-xl bg-white dark:bg-slate-900 w-full">
        <CardHeader className="border-b pb-4 flex flex-row items-center justify-between">
          <div>
            <CardTitle className="text-lg font-bold flex items-center gap-2">
              <Activity className="h-5 w-5 text-indigo-500" /> Live Spend Trajectory (Historical vs AI Projections)
            </CardTitle>
            <CardDescription className="text-xs">Continuous area forecast showing actual synced ledger spends transitioning into future monthly estimates.</CardDescription>
          </div>
          ...
          <div className="h-[360px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                ...
                />
                <Area type="monotone" dataKey="Historical Spend" stroke="#6366f1" strokeWidth={2.5} fillOpacity={1} fill="url(#colorHist)" />
                <Area type="monotone" dataKey="Forecasted Spend" stroke="#a855f7" strokeWidth={2.5} strokeDasharray="5 5" fillOpacity={1} fill="url(#colorFore)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {/* Chart 2: Vendor Forecast comparison (Growth vs Risk) */}
        <Card className="border-0 shadow-xl bg-white dark:bg-slate-900">
          <CardHeader>
            <CardTitle className="text-base font-bold flex items-center gap-2">
              <BarChart3 className="h-5 w-5 text-emerald-500" /> Supplier Forecasted Growth vs Concentration Risk
            </CardTitle>
            <CardDescription className="text-xs">Est growth in raw purchase mappings compared to single-supplier dependency scores.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-[280px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data?.supplierForecast || []} margin={{ top: 10, right: 10, left: 10, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} className="stroke-slate-200 dark:stroke-slate-800" />
                  <XAxis dataKey="supplier" className="text-[9px] text-slate-400 font-semibold" tickLine={false} />
                  <YAxis className="text-[9px] text-slate-400" tickLine={false} axisLine={false} />
                  <Tooltip formatter={(value) => [`${value}% / Score`, '']} />
                  <Legend wrapperStyle={{ fontSize: '10px' }} />
                  <Bar dataKey="predictedGrowth" name="Projected Spend Growth %" fill="#10b981" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="riskScore" name="Concentration Risk Index" fill="#f43f5e" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        {/* Chart 3: Product Purchase Horizontal comparison */}
        <Card className="border-0 shadow-xl bg-white dark:bg-slate-900">
          <CardHeader>
            <CardTitle className="text-base font-bold flex items-center gap-2">
              <Layers className="h-5 w-5 text-indigo-500" /> Material Purchase Demand Rates
            </CardTitle>
            <CardDescription className="text-xs">Projected next-quarter procurement growth rates by stock item category.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-[280px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart layout="vertical" data={data?.productPredictions || []} margin={{ top: 10, right: 20, left: 20, bottom: 5 }}>
                  ...
                  <Bar dataKey="growthRate" name="Demand Growth Rate" fill="#8b5cf6" radius={[0, 4, 4, 0]}>
      ...
      {/* Supplier Projections & Product Predictions Tables */}
      <div className="grid gap-6 md:grid-cols-2">
        {/* Supplier Forecast Table */}
        <Card className="border-0 shadow-lg bg-white dark:bg-slate-900">
          <CardHeader>
            <CardTitle className="text-base font-bold flex items-center gap-2">
              <Users className="h-5 w-5 text-indigo-500" /> Supplier Performance Forecast
            </CardTitle>
            <CardDescription className="text-xs">Estimated vendor procurement volumes growth rates and dependency risk index scores.</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full text-xs">
                <thead className="bg-slate-50 dark:bg-slate-800 text-slate-400 uppercase font-bold text-[10px]">
                  <tr>
                    ...
                    <th className="text-right p-3">Est Spend Growth</th>
                    <th className="text-right p-3 font-semibold">Dependency Risk</th>
                  </tr>
                </thead>
                <tbody className="divide-y text-slate-600 dark:text-slate-300">
                  {(data?.supplierForecast || []).map((s: any, idx: number) => {
                    const getRiskBadge = (score: number) => {
                      if (score > 60) return <Badge className="bg-rose-50 text-rose-600 dark:bg-rose-950/20 dark:text-rose-400 border-rose-100 text-[10px]">High Concentration</Badge>;
                      if (score > 35) return <Badge className="bg-amber-50 text-amber-600 dark:bg-amber-950/20 dark:text-amber-400 border-amber-100 text-[10px]">Medium Dependency</Badge>;
                      return <Badge className="bg-emerald-50 text-emerald-600 dark:bg-emerald-950/20 dark:text-emerald-400 border-emerald-100 text-[10px]">Stable Vendor</Badge>;
                    };
                    return (
                      <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20">
                        <td className="p-3 font-bold text-slate-800 dark:text-slate-200">{s.supplier}</td>
                        <td className="p-3 text-right font-bold text-emerald-600">+{s.predictedGrowth}%</td>
                        <td className="p-3 text-right">{getRiskBadge(s.riskScore)}</td>
                      </tr>
                    );
                  })}
                </tbody>
        ...
        <Card className="border-0 shadow-lg bg-white dark:bg-slate-900">
          <CardHeader>
            <CardTitle className="text-base font-bold flex items-center gap-2">
              <Package className="h-5 w-5 text-indigo-500" /> Material Projections Breakdown
            </CardTitle>
            <CardDescription className="text-xs">Purchased items category volume growth forecasts and demand cycles.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="rounded-xl border overflow-hidden">
              <table className="w-full text-xs">
                <thead className="bg-slate-50 dark:bg-slate-800 text-slate-400 uppercase font-bold text-[10px]">
                  <tr>
                    <th className="text-left p-3">Material Category</th>
                    <th className="text-left p-3">Purchases Status</th>
                    <th className="text-right p-3 font-semibold">Est Spend Growth</th>
                  </tr>
                </thead>
                <tbody className="divide-y text-slate-600 dark:text-slate-300">
                  {(data?.productPredictions || []).map((p: any, idx: number) => {
                    const getStatusColor = (status: string) => {
                      if (status.includes("High Volume")) return "text-emerald-600 bg-emerald-50 dark:bg-emerald-950/20 px-2 py-0.5 rounded font-bold border border-emerald-100";
      ...
      <Card className="border-0 shadow-lg bg-white dark:bg-slate-900">
        <CardHeader>
          <CardTitle className="text-base font-bold flex items-center gap-2 text-indigo-700 dark:text-indigo-400">
            <CheckCircle2 className="h-5 w-5 text-indigo-600" /> Actionable Procurement Recommendations
          </CardTitle>
          <CardDescription className="text-xs">Actionable cost negotiation and supply chain buffer strategies based on Tally spend records.</CardDescription>
        </CardHeader>
          ...
          <div className="bg-white dark:bg-slate-900 border dark:border-slate-800 rounded-2xl shadow-2xl max-w-xl w-full p-6 space-y-4 animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b pb-3">
              <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                ...
                AI Procurement Projections Logic
              </h3>
              <button 
            ...
            <div className="space-y-4 text-xs leading-relaxed text-slate-600 dark:text-slate-300 max-h-[60vh] overflow-y-auto pr-2">
              ...
              <div className="space-y-1.5">
                <h4 className="font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wide text-[10px] flex items-center gap-1.5">
                  ...
                  2. Vendor Concentration & Delivery Risks
                </h4>
