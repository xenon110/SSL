...
export default function CashFlowDashboard() {
  ...
  const [modalType, setModalType] = useState<'all' | 'income' | 'expense' | 'liquidity_bank' | 'liquidity_cash' | null>(null);
  ...
  const triggerSync = async () => {
    try {
      ...
      let dataUrl = '/api/income-expense';
  ...
  const allLedgersCombined = useMemo(() => {
    ...
    (data.topIncomes || []).forEach((s: any) => {
      map[s.name] = { name: s.name, income: s.amount, expense: 0, net: s.amount };
    });
    (data.topExpenses || []).forEach((u: any) => {
      if (!map[u.name]) map[u.name] = { name: u.name, income: 0, expense: 0, net: 0 };
      map[u.name].expense += u.amount;
      map[u.name].net -= u.amount;
    });
  ...
  const predictionUrl = useMemo(() => {
    let url = '/income-expense/prediction';
    const params = new URLSearchParams();
    if (dateRange?.from) params.append('startDate', format(dateRange.from, 'yyyy-MM-dd'));
    if (dateRange?.to) params.append('endDate', format(dateRange.to, 'yyyy-MM-dd'));
    if (params.toString()) url += '?' + params.toString();
      ...
      {(isLoading || !data) ? (
        <div className="flex flex-1 items-center justify-center h-[500px]">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-lg font-medium text-slate-600 animate-pulse">Visualizing Income & Expense...</p>
          </div>
        </div>
      ) : (
        <div className="space-y-8 animate-in slide-in-from-bottom-8 duration-700 fade-in fill-mode-both">

          {/* Expected Income & Expense Strip */}
          {expectedFlows && (
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            ...
            >
              ...
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 relative z-10">
                <CardTitle className="text-sm font-semibold uppercase tracking-wider text-slate-500">Net Income & Expense</CardTitle>
              ...
              <CardContent className="relative z-10">
                <div className={`text-3xl font-extrabold tracking-tight ${kpis.netProfit >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                  {formatCompact(kpis.netProfit)}
                </div>
            ...
            >
              ...
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 relative z-10">
                <CardTitle className="text-sm font-semibold uppercase tracking-wider text-slate-500">Total Income (Receipts)</CardTitle>
              ...
              <CardContent className="relative z-10">
                <div className="text-3xl font-extrabold text-slate-800 dark:text-white tracking-tight">{formatCompact(kpis.totalIncome)}</div>
                <p className="text-xs text-emerald-600 mt-1 font-medium flex items-center gap-1">View Top Incomes <ArrowUpRight className="h-3 w-3 ml-auto opacity-50" /></p>
              </CardContent>
            </Card>
            ...
            >
              ...
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 relative z-10">
                <CardTitle className="text-sm font-semibold uppercase tracking-wider text-slate-500">Total Expense (Payments)</CardTitle>
              ...
              <CardContent className="relative z-10">
                <div className="text-3xl font-extrabold text-slate-800 dark:text-white tracking-tight">{formatCompact(kpis.totalExpense)}</div>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-2">
            ...
            <Card 
              className="relative overflow-hidden border-0 shadow-xl bg-gradient-to-br from-indigo-500 to-blue-600 transition-all hover:shadow-2xl hover:-translate-y-1 duration-300 group"
            >
              ...
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 relative z-10">
                <CardTitle className="text-sm font-semibold uppercase tracking-wider text-blue-100">Operating Margin</CardTitle>
              ...
              <CardContent className="relative z-10">
                <div className="text-4xl font-black text-white tracking-tight">{(kpis.margin || 0).toFixed(2)}%</div>
                <p className="text-xs text-blue-100 mt-1 font-medium flex items-center gap-1">Net profit margin of operating revenue</p>
              </CardContent>
            </Card>
          ...
          {/* Net Profit/Loss (Surplus / Deficit) Chart */}
              ...
              <Card className="border-0 shadow-xl bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl flex flex-col lg:col-span-2">
                <CardHeader className="border-b border-slate-100 dark:border-slate-800 pb-4">
                  <CardTitle className="text-lg font-bold flex items-center gap-2 text-slate-800 dark:text-white">
                    ...
                    Net Profit/Loss (Surplus / Deficit)
                  </CardTitle>
                  <CardDescription className="text-slate-500">Actual net income and expense (Incomes minus Expenses) synced from Tally for the selected date range</CardDescription>
                </CardHeader>
                <CardContent className="h-[350px] p-4 relative">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data.trendData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                      ...
                      <Bar name="Net Cash Movement" dataKey="net" radius={[4,4,0,0]}>
          ...
          {/* Row 2: Main Income & Expense Charts */}
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-2">
            <Card className="border-0 shadow-xl bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl flex flex-col lg:col-span-2">
              <CardHeader className="flex-none">
                ...
                <CardDescription className="text-sm">Income vs Expense over time</CardDescription>
              </CardHeader>
              <CardContent className="h-[400px]">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={data.trendData} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                    ...
                    <Legend />
                    <Area type="monotone" dataKey="income" name="Cash Income" stroke="#10b981" strokeWidth={3} fillOpacity={1} fill="url(#colorIn)" />
                    <Area type="monotone" dataKey="expense" name="Cash Expense" stroke="#f43f5e" strokeWidth={3} fillOpacity={1} fill="url(#colorOut)" />
                  </AreaChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
            ...
            <Card className="border-0 shadow-xl bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl flex flex-col">
              ...
              <CardContent className="flex-1 overflow-hidden h-[350px]">
                <div className="h-full overflow-y-auto pr-2 custom-scrollbar" style={{ maxHeight: '350px' }}>
                  <ResponsiveContainer width="100%" height={Math.max(100, (data.topIncomes?.slice(0, 20).length || 0) * 45 + 40)}>
                    <BarChart data={data.topIncomes?.slice(0, 20) || []} layout="vertical" margin={{ top: 5, right: 40, left: 10, bottom: 5 }}>
            ...
            <Card className="border-0 shadow-xl bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl flex flex-col">
              <CardHeader className="flex-none">
                <CardTitle className="text-xl font-bold flex items-center gap-2">
                  ...
                  Top Expenses (Payments)
                </CardTitle>
                <CardDescription className="text-sm">Click any bar to open ledger deep dive</CardDescription>
              </CardHeader>
              <CardContent className="flex-1 overflow-hidden h-[350px]">
                <div className="h-full overflow-y-auto pr-2 custom-scrollbar" style={{ maxHeight: '350px' }}>
                  <ResponsiveContainer width="100%" height={Math.max(100, (data.topExpenses?.slice(0, 20).length || 0) * 45 + 40)}>
                    <BarChart data={data.topExpenses?.slice(0, 20) || []} layout="vertical" margin={{ top: 5, right: 40, left: 10, bottom: 5 }}>
                      <CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} stroke="#f1f5f9" />
                      <XAxis type="number" fontSize={12} stroke="#94a3b8" tickFormatter={(v) => formatCompact(v)} />
                      <YAxis dataKey="name" type="category" width={180} fontSize={11} fontWeight={500} tickFormatter={(v: string) => v.length > 22 ? v.substring(0, 20) + '…' : v} tick={{ fill: '#475569' }} />
                      <RechartsTooltip cursor={{ fill: '#f1f5f9' }} formatter={(value: any) => formatCurrency(value)} />
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b bg-slate-50 dark:bg-slate-800/50">
              <div>
                <h3 className="text-2xl font-bold flex items-center gap-2 text-slate-800 dark:text-white">
                  {modalType === 'income' ? <ArrowDownRight className="h-6 w-6 text-emerald-600" /> :
                   modalType === 'expense' ? <ArrowUpRight className="h-6 w-6 text-rose-600" /> :
                   <Banknote className="h-6 w-6 text-indigo-600" />}
                  {modalType === 'income' ? 'Cash Income Distribution' :
                   modalType === 'expense' ? 'Cash Expense Distribution' :
                   modalType === 'liquidity_bank' ? 'Bank Accounts Detail' :
                   modalType === 'liquidity_cash' ? 'Cash-in-Hand Detail' :
                   'Master Ledger Grid'}
                </h3>
                <p className="text-sm text-slate-500 mt-1">
                  {modalType === 'all' ? 'Interactive cards for all income-expense ledgers. Click to deep dive.' :
                   modalType === 'liquidity_bank' || modalType === 'liquidity_cash' ? 'Live closing balances directly from Tally.' :
                   'Donut chart visualization of your highest income and expense sources. Click a slice to deep dive.'}
                </p>
              </div>
                      ...
                      <CardContent className="p-4 pt-0">
                        <div className="flex justify-between items-end mt-4">
                          <div className="space-y-1">
                            ...
                            <p className={`text-xl font-black ${p.net >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{formatCompact(p.net)}</p>
                          </div>
                          <div className="text-right space-y-1">
                            <p className="text-[10px] uppercase font-bold tracking-wider text-emerald-500">IN: {formatCompact(p.income)}</p>
                            <p className="text-[10px] uppercase font-bold tracking-wider text-rose-500">OUT: {formatCompact(p.expense)}</p>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
                ...
                <div className="h-[500px] w-full flex items-center justify-center">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      ...
                      <Pie
                        data={
                          (modalType === 'income' ? data?.topIncomes : data?.topExpenses)
                            ?.slice(0, 15)
                      ...
                      >
                        {Array.from({length: 15}).map((_, index) => (
                          <Cell key={`cell-${index}`} fill={modalType === 'income' ? donutColors[index % donutColors.length] : donutColors.reverse()[index % donutColors.length]} className="hover:opacity-80 transition-opacity" />
                        ))}
                      </Pie>
                    </PieChart>
                  </ResponsiveContainer>
            ...
            <div className="flex items-center justify-between px-8 py-6 border-b bg-gradient-to-r from-emerald-50 to-white dark:from-slate-800 dark:to-slate-900">
              ...
              <div className="grid grid-cols-3 gap-6">
                <Card className="border-0 shadow-md bg-white">
                  ...
                  <CardContent>
                    <p className="text-3xl font-black text-emerald-600">{formatCurrency(selectedLedger.income || selectedLedger.amount || 0)}</p>
                  </CardContent>
                </Card>
                <Card className="border-0 shadow-md bg-white">
                  <CardHeader className="pb-2">
                  ...
                  <CardContent>
                    <p className="text-3xl font-black text-rose-600">{formatCurrency(selectedLedger.expense || 0)}</p>
                  </CardContent>
                </Card>
                <Card className="border-0 shadow-md bg-white relative overflow-hidden">
                  ...
                  <CardHeader className="pb-2 relative z-10">
                    <CardTitle className="text-xs uppercase text-slate-400 font-bold">Net Income & Expense</CardTitle>
                  </CardHeader>
                  <CardContent className="relative z-10">
                    <p className={`text-4xl font-black ${(selectedLedger.net || selectedLedger.amount || 0) >= 0 ? 'text-indigo-700' : 'text-rose-700'}`}>
                      {formatCurrency(selectedLedger.net || selectedLedger.amount || 0)}
                    </p>
                  </CardContent>
                </Card>
              </div>
              ...
              <Card className="border-0 shadow-xl bg-white flex flex-col overflow-hidden">
                <div className="px-6 py-4 border-b flex justify-between items-center bg-slate-50/80">
                  <div>
                    <h4 className="text-lg font-bold flex items-center gap-2"><Activity className="h-5 w-5 text-indigo-500" /> Historical Income & Expense</h4>
                    <p className="text-xs text-slate-500">Track and compare the monthly income and expense with this ledger.</p>
                  </div>
                  <div className="flex items-center gap-3">
