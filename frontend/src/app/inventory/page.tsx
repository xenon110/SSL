...
export default function InventoryDashboard() {
  ...
  const fetchMetrics = async (startDate?: string, endDate?: string) => {
    ...
    setError(null);
    try {
      const url = new URL('/api/universal-metrics?type=inventory', window.location.origin);
  ...
  return (
    <div className="p-6 space-y-6">
      <GenericDashboardView title="Inventory Dashboard" data={data} onDateChange={(start, end) => fetchMetrics(start, end)} isLoading={isLoading} />
    </div>
  );
}
