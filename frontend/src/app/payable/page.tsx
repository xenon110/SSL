...
import { 
  Building2, Landmark, Wallet, AlertTriangle, FileText, X, Search, ChevronRight, ChevronDown, Phone, Mail, Clock, ShieldAlert, ArrowRight, TrendingDown
} from "lucide-react";
...
export default function Page() {
  ...
  // Selected Vendor (Level 3 Drawer)
  ...
  // Aggregate Payables data by Parent Group
  const groupedData = useMemo(() => {
    if (!rawData?.payables) return [];
    
    const groups: Record<string, any> = {};
    rawData.payables.forEach((r: any) => {
      // Search filter
      const matchesSearch = r.name.toLowerCase().includes(search.toLowerCase()) ||
                            (r.parentGroup || "").toLowerCase().includes(search.toLowerCase());
      if (search && !matchesSearch) return;

      const groupName = r.parentGroup || "Sundry Creditors";
      if (!groups[groupName]) {
        groups[groupName] = {
          name: groupName,
          totalPending: 0,
  ...
  const totalOutstanding = rawData?.kpis?.totalPayables || 0;
  const overdueOutstanding = rawData?.kpis?.overduePayables || 0;
  ...
  if (loading) {
    return (
      <div className="flex h-[500px] items-center justify-center flex-col gap-4">
        ...
        <p className="text-sm font-semibold text-slate-500 tracking-wider">Loading Accounts Payable Register...</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-8 max-w-[1700px] mx-auto bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-4 border-slate-200">
        <div>
          <h1 className="text-3xl font-black bg-clip-text text-transparent bg-gradient-to-r from-orange-600 to-amber-500">
            Accounts Payable Ledger
          </h1>
          <p className="text-sm text-slate-500 font-medium mt-1">
            Hierarchical drill-down of outstanding liabilities — synced live from Tally
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="bg-white border-0 shadow-lg relative overflow-hidden group">
          ...
          <CardContent className="p-6 relative z-10">
            <div className="flex justify-between items-start">
              ...
              <div className="p-3 bg-orange-100 rounded-xl text-orange-600"><Landmark className="h-6 w-6" /></div>
            </div>
            <p className="text-xs text-orange-600 font-semibold mt-4">Total active vendor payables</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white border-0 shadow-lg relative overflow-hidden group">
          ...
          <CardContent className="p-6 relative z-10">
            ...
            <p className="text-xs text-rose-600 font-semibold mt-4">Liabilities past due date</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-orange-500 to-orange-700 border-0 shadow-lg text-white">
          <CardContent className="p-6">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-orange-100 text-xs font-bold uppercase tracking-wider mb-2">Cash Flow Impact</p>
                <h3 className="text-4xl font-black">{formatCompact(totalOutstanding)}</h3>
              </div>
              <div className="p-3 bg-orange-600/30 rounded-xl text-orange-100"><TrendingDown className="h-6 w-6" /></div>
            </div>
            <p className="text-xs text-orange-100/80 font-semibold mt-4">Total cash outflow requirement</p>
          </CardContent>
        </Card>
      </div>
          ...
          <Table>
            <TableHeader className="bg-slate-50 border-b border-slate-200">
              <TableRow className="border-none hover:bg-transparent">
                ...
                <TableHead className="font-bold text-slate-700 text-sm">Vendor / Group Name</TableHead>
                  ...
                  return (
                    <React.Fragment key={group.name}>
                      ...
                      <TableRow 
                        ...
                        className="cursor-pointer bg-orange-50/20 hover:bg-orange-50/40 transition-colors border-b border-slate-100 font-semibold"
                      >
                        ...
                        <TableCell className="text-orange-950 font-bold text-base flex items-center gap-2">
                          <Building2 className="h-4.5 w-4.5 text-orange-500 shrink-0" />
                          {group.name}
                          <Badge variant="outline" className="ml-2 text-orange-600 border-orange-200 bg-orange-50/50">
                      ...
                      {/* Level 2: Vendors under this Group */}
                      {isExpanded && group.parties.map((party: any) => (
                        ...
                        >
                          <TableCell></TableCell>
                          <TableCell className="pl-6 py-3.5 font-bold text-slate-700 group-hover:text-orange-600 flex items-center justify-between">
                            <span className="truncate max-w-sm">{party.name}</span>
                            <ArrowRight className="h-4 w-4 opacity-0 group-hover:opacity-100 transition-opacity text-orange-500 mr-4 shrink-0" />
                          </TableCell>
      ...
      {/* Level 3: Pending Invoices Drawer */}
      {selectedParty && (
            ...
            <div className="pointer-events-auto w-screen max-w-3xl transform bg-white shadow-2xl transition-transform duration-300 ease-in-out animate-in slide-in-from-right">
              ...
              <div className="p-6 space-y-6 overflow-y-auto h-[calc(100vh-130px)]">
                ...
                <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
                  <div className="space-y-1.5">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-bold text-slate-700">Credit Period:</span>
                      <Badge className="bg-orange-100 text-orange-700 hover:bg-orange-100">{selectedParty.creditDays || 0} days</Badge>
                    </div>
                  </div>
                ...
                <div className="space-y-3">
                  <h4 className="text-xs text-slate-400 font-bold uppercase tracking-wider">Outstanding Invoices</h4>
                        ...
                        {selectedParty.bills.length === 0 ? (
                          <TableRow>
                            <TableCell colSpan={5} className="text-center py-6 text-slate-400 font-semibold">
                              No outstanding invoices on account.
                            </TableCell>
                          </TableRow>
                        ) : (
                            ...
                            <TableRow key={idx} className="border-b last:border-0 hover:bg-slate-50">
                              ...
                              <TableCell className="text-xs font-medium uppercase">
                                <Badge variant="outline" className={`text-[10px] ${
                                  bill.bill_type === 'advance' ? 'bg-orange-50 border-orange-200 text-orange-700' : 'bg-slate-50 border-slate-200 text-slate-700'
                                }`}>
                                  {bill.bill_type}
                                </Badge>
                              </TableCell>
