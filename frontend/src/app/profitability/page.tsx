...
export default function ProfitabilityPage() {
  ...
  const fetchMetrics = async (startDate?: string, endDate?: string) => {
    ...
    setError(null);
    try {
      const url = new URL('/api/universal-metrics?type=profitability', window.location.origin);
  ...
  return (
    <div className="p-6 space-y-6">
      <GenericDashboardView title="Profitability Dashboard" data={data} onDateChange={(start, end) => fetchMetrics(start, end)} isLoading={isLoading} />
    </div>
  );
}
